<template>
  <q-page padding>
    ...
    <AppTable
      ...
      :on-row-click="(row) => $router.push({ name: 'appointment-detail', params: { id: row.id } })"
      :rows="filteredRows"
      @delete="(item) => handleDelete(item, { label: () => 'Bu randevu' })"
    >
      <template #cell-appointmentDate="props">
        <q-td :props="props">{{ formatDate(props.value) }}</q-td>
      </template>
      <template #cell-status="props">
        <q-td :props="props">{{ statusLabel(props.value) }}</q-td>
      </template>
      <template #cell-fee="props">
        <q-td :props="props">{{ props.value ? props.value + ' ₺' : '-' }}</q-td>
      </template>
      <template #actions="props">
        <q-td :props="props" auto-width>
          ...
          <q-btn color="negative" dense flat label="Sil" size="sm"
                 @click.stop="handleDelete(props.row, { label: () => 'Bu randevu' })"/>
        </q-td>
      </template>
    </AppTable>
    ...
    >
      <q-form ref="bookFormRef" class="q-gutter-sm" @submit="handleBook">
        <q-select v-model="bookForm.patientId" :options="patientOptions" :rules="[v => !!v || 'Zorunlu']"
                  dense emit-value input-debounce="0" label="Hasta *" map-options outlined use-input/>
        <q-select v-model="bookForm.doctorId" :options="doctorOptions" :rules="[v => !!v || 'Zorunlu']"
                  dense emit-value input-debounce="0" label="Doktor *" map-options outlined use-input/>
...
const filterConfig = [
  {key: 'search', type: 'search', label: 'Hasta / Doktor', width: '220px'},
